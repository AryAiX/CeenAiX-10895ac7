...
| Supabase Auth | Branded auth email HTML (all auth mailers, all roles) | Dev + prod | `sync-dev-auth-platform.sh` (Build) / `sync-prod-auth-platform.sh` (Release) | done (CI) | Dev sync on **Build** after merge to `main`. Prod sync on manual **Release** only. |
...
| Supabase Auth | Site URL for auth emails and redirects | Dev + prod | `dev-platform-config.env` / `prod-platform-config.env` | done (CI) | Dev: **Build** on `main` push. Prod: manual **Release**. |
| Supabase Auth | Redirect URL allow-list for final domains | Dev + prod | platform config env files + CI workflows | done (CI) | Dev allow-list includes `dev.ceenaix.com`, previews, localhost. Prod includes www, apex, previews, localhost. |
