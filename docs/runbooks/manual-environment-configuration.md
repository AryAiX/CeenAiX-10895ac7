...
| Supabase Auth | Branded auth email HTML (all auth mailers, all roles) | Dev + prod | `sync-dev-auth-platform.sh` / `sync-prod-auth-platform.sh` via Release workflow | done (CI) | **One shared layout for every role.** Dev sync on every `main` push (`dev.ceenaix.com`). Prod sync on manual **Run workflow** only. |
...
| Supabase Auth | Site URL for auth emails and redirects | Dev + prod | `dev-platform-config.env` / `prod-platform-config.env` | done (CI) | Dev: `https://dev.ceenaix.com` on main push. Prod: `https://www.ceenaix.com` on manual Release. |
| Supabase Auth | Redirect URL allow-list for final domains | Dev + prod | platform config env files + Release workflow | done (CI) | Dev allow-list includes `dev.ceenaix.com`, previews, localhost. Prod includes www, apex, previews, localhost. |
