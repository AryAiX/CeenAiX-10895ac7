  ...
  return useQuery<LabPortalData | null>(async () => {
    ...
    return {
      ...
      metrics: {
        ...
        labOrders: samples.filter((sample) => sample.status === 'ordered').length,
        ...
        imagingQueue: imagingStudies.filter((study) => study.status === 'scanning' || study.status === 'scheduled' || study.status === 'report_pending').length,
        imagingOrders: imagingStudies.filter((study) => study.status === 'ordered').length,
