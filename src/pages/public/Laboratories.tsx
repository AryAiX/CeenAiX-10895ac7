...
export const Laboratories: React.FC = () => {
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-b from-slate-50 via-white to-white">
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 z-0 h-[22rem] overflow-hidden sm:h-[26rem]" aria-hidden>
        <img
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        {loading ? (
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {[...Array(6)].map((_, index) => (
              <div key={index} className="animate-pulse rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="col-span-full text-center text-sm text-slate-500">{t('laboratoryPage.loading')}</p>
          </div>
        ) : filteredLaboratories.length === 0 ? (
          <div className="rounded-[2rem] border border-slate-200 bg-white py-12 text-center shadow-sm">
            <Beaker className="mx-auto mb-4 h-16 w-16 text-slate-400" />
            <p className="text-lg text-slate-600">{t('laboratoryPage.emptyState')}</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              ...
              return (
                <div
                  ...
                  className={`card-hover overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm ${
                    lab.featured ? 'ring-2 ring-cyan-500' : ''
                  }`}
                >
                  {lab.featured && (
                  ...
                  <div className="p-6">
                    <div className="mb-4 flex items-start justify-between">
                      <div className="flex-1">
                        <h3 className="mb-1 text-xl font-bold text-slate-900">{nameDisplay}</h3>
                    ...
                    <div className="mb-4 flex items-center text-slate-700">
                      <Clock className="me-2 h-4 w-4 text-slate-500" />
                      <span className="text-sm">{hoursDisplay}</span>
                    </div>

                    <div className="mb-4 flex items-center text-slate-700">
                      <TestTube className="me-2 h-4 w-4 text-cyan-700" />
                    ...
                    <button
                      ...
                      className="w-full rounded-xl bg-gradient-to-r from-ceenai-cyan to-ceenai-blue py-3 font-semibold text-white shadow-md transition-all hover:shadow-lg hover:shadow-cyan-500/30"
                    >
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            <div className="text-center">
              <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-cyan-100">
                <Beaker className="h-6 w-6 text-cyan-700" />
              </div>
              <h3 className="mb-2 font-semibold text-slate-900">{t('laboratoryPage.benefit1Title')}</h3>
              <p className="text-sm text-slate-600">{t('laboratoryPage.benefit1Body')}</p>
            </div>
            <div className="text-center">
              <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-green-100">
                <TestTube className="h-6 w-6 text-green-600" />
              </div>
              <h3 className="mb-2 font-semibold text-slate-900">{t('laboratoryPage.benefit2Title')}</h3>
              <p className="text-sm text-slate-600">{t('laboratoryPage.benefit2Body')}</p>
            </div>
            <div className="text-center">
              <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-blue-100">
                <Clock className="h-6 w-6 text-blue-600" />
              </div>
              <h3 className="mb-2 font-semibold text-slate-900">{t('laboratoryPage.benefit3Title')}</h3>
              <p className="text-sm text-slate-600">{t('laboratoryPage.benefit3Body')}</p>
            </div>
          </div>
        </div>
      </div>
